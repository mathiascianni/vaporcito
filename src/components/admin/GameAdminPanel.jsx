...
const GameAdminPanel = ({ games, loading }) => {
    ...
    const handleDelete = async (game) => {
        ...
        setShowModal(false);
        // Crear modal de creado correctamente
        navigate("/")
    }

    const handleModal = (game) => {
                        ...
                        <div className="flex-1 p-4 md:overflow-x-scroll flex gap-4 md:gap-8 flex-col md:flex-row">
                            ...
                            <div className="flex flex-col h-full md:w-[300px] pl-2 border-l border-input-light">
                                ...
                                <div className="flex-1 flex items-center h-full gap-2">
                                    ...
                                    <button className="inline-block border border-error text-error p-1 px-4 rounded-full hover:bg-error hover:text-white transition w-full md:w-auto text-center" onClick={() => handleModal(game)}>Eliminar</button>
                                </div>
                            </div>
                        </div>
                        {showModal && <ModalNotification title="¿Deseas eliminar este juego?" subtitle="El juego se eliminará para siempre (¡Eso es mucho tiempo!)." icon={<FiAlertCircle className="text-4xl text-error" />} tBtnText="No, cancelar" tBtnAction={() => setShowModal(false)} fBtnText="Sí, eliminar" fBtnAction={() => handleDelete(gameToDelete)} close={() => setShowModal(false)} iColor />}
                    </div>
                ))
            }
        </>
